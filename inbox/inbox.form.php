...
if ($sesi_username != NULL && !empty($sesi_username) && $_SESSION['leveluser'] <= '8') {
...
?>
	...
	<form action="?id=msg" id="form-inbox" method="POST">
		...
		<div class="form-group">
			...
			<div class="">
			<select name="username" class="chosen-select form-control" id="form-field-sdata-placeholder="Silahkan Pilih Username... required="">
			<option value=" ">  </option>
			<?php
			$level_user = $_SESSION['leveluser'];
			if ($level_user == 5) {
                $q = mysqli_query($mysqli, "SELECT username FROM tbl_user WHERE level_user = 1");
            } else {
