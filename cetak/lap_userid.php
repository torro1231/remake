...
if ($sesi_username != NULL || !empty($sesi_username)) {
    ...
    while ($data1 = mysqli_fetch_row($query)) {
        ...
        $hasil = mysqli_query($mysqli, "SELECT username,nip,user.n_lv FROM tbl_user,user where tbl_user.level_user=user.level_user order by w_daftar desc");
        $no = 1;

        while ($row = mysqli_fetch_array($hasil)) {
            $pdf->SetFillColor(255, 255, 255);
            $pdf->Cell(1, 0.8, $no++, 1, 0, 'C', true);
            $pdf->Cell(7, 0.8, $row[0], 1, 0, 'C', true);
            $pdf->Cell(3, 0.8, $row[1], 1, 0, 'L', true);
            $pdf->Cell(4, 0.8, $row[2], 1, 0, 'L', true);
            $pdf->Ln();
        }
        ...
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->SetX(1);
        $pdf->MultiCell(25, 0.8, 'Siswantoro', 0, 'R');
