...
$no = 1;
$hasil = mysqli_query($mysqli, "select nip, nama, klien from pegawai where klien='$penempatan' and kdstatusp NOT IN ('0','4') ");
while ($data = mysqli_fetch_row($hasil)) {
	$nip_a	= $data[0];
	$nama	= $data[1];
	$klien = $data[2];

	$hasilhadir = mysqli_query($mysqli, "SELECT count(*) as jum FROM absensi WHERE id_ijin ='0' and tanggal_absen between '$p_awal' and '$p_akhir' and nip='$nip_a' and penempatan='$klien' ");
	...
	$hasilcuti = mysqli_query($mysqli, "SELECT  sum(lama_cuti) as jum FROM ajuancuti WHERE tgl_mulai between '$p_awal' and '$p_akhir' and nip='$nip_a'");
