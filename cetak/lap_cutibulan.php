...
while ($data = mysqli_fetch_row($hasil)){
	  ...
	  $queryCM = mysqli_query($mysqli,"SELECT sum(lama_cuti) as jum FROM ajuancuti WHERE id_cuti ='4' and kd_approve='1' and month(tgl_mulai)='$p_bulan' and year(tgl_mulai)='$p_tahun' and nip='$nip'  ");
      $rowCM = mysqli_fetch_row($queryCM);
      $hasilCM = $rowCM[0];
	  
	  //query untuk menghitung CN
	  $queryCN = mysqli_query($mysqli,"SELECT sum(lama_cuti)  as jum FROM ajuancuti WHERE id_cuti ='7' and kd_approve='1' and month(tgl_mulai)='$p_bulan' and year(tgl_mulai)='$p_tahun'  and nip='$nip' ");
      $rowCN = mysqli_fetch_row($queryCN);
      $hasilCN = $rowCN[0];
	  
	  //query untuk menghitung CS
	  $queryCS = mysqli_query($mysqli,"SELECT sum(lama_cuti)  as jum FROM ajuancuti WHERE id_cuti ='3' and kd_approve='1' and month(tgl_mulai)='$p_bulan' and year(tgl_mulai)='$p_tahun'  and nip='$nip' ");
      $rowCS = mysqli_fetch_row($queryCS);
      $hasilCS = $rowCS[0];
	  
	  //query untuk menghitung CT
	  $queryCT = mysqli_query($mysqli,"SELECT sum(lama_cuti)  as jum FROM ajuancuti WHERE id_cuti ='1' and kd_approve='1' and month(tgl_mulai)='$p_bulan' and year(tgl_mulai)='$p_tahun'  and nip='$nip'  ");
      $rowCT = mysqli_fetch_row($queryCT);
      $hasilCT = $rowCT[0];
	  
	  //query untuk menghitung CB
	  $queryCAP = mysqli_query($mysqli,"SELECT sum(lama_cuti)  as jum FROM ajuancuti WHERE id_cuti ='2' and kd_approve='1' and month(tgl_mulai)='$p_bulan' and year(tgl_mulai)='$p_tahun'  and nip='$nip'  ");
