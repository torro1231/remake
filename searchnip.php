...
if (isset($_POST['search_keyword'])) {
    ...
    if ($rows_returned > 0) {
        while ($row = $res->fetch_assoc()) {
			...
			<span id="nip" class="nip">' . str_ireplace($search_keyword, $bold_search_keyword, $row['nip']) . '</span>&nbsp 
			<span id="nama" class="nama">' . str_ireplace($search_keyword, $bold_search_keyword, $row['nama']) . '</span>&nbsp
            <span id="penempatan" class="penempatan">' . str_ireplace($search_keyword, $bold_search_keyword, $row['klien']) . '</span></div>';
        }
    } else {
