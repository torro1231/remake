...
if ($mod == "del") {
	$q_delete_berkas = mysqli_query($mysqli, "DELETE FROM tbl_berkas WHERE id_berkas = '$id_berkas'");
	if ($q_delete_berkas) {
...
if ($tb_act == "Tambah") {
	$display = "style='display: none'";
	$q_tambah	= mysqli_query($mysqli, "INSERT INTO tbl_berkas VALUES ('0', '$p_nm_berkas')");
	if ($q_tambah) {
		...
		while ($a_berkas = mysqli_fetch_array($q_berkas)) {
			echo "<tr>
				...
				<td id='tengah'>
					...
					<a href='?id=berkas&mod=del&id_berkas=$a_berkas[0]' onclick=\"return confirm('Menghapus data $a_berkas[1]')\"><span class='red'><i class='ace-icon fa fa-trash-o bigger-120'></i></span></a>
				</tr>";
			$no++;
		}
	}
...
<div <?php echo $display; ?>>
	<header>
		<h3 class="header smaller lighter blue"><?php echo $view; ?> Data Berkas</h3>
	</header>
