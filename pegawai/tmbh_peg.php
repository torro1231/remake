...
if ($sesi_username != NULL || !empty($sesi_username) || $_SESSION['leveluser'] == '1' || $_SESSION['leveluser'] == '2') {
    ...
    $p_jnspeg        = isset($_POST['id_jns']) ? $_POST['id_jns'] : 0;
    ...
    $p_kd_sub        = isset($_POST['kd_sub']) ? intval($_POST['kd_sub']) : 0;
    ...
    $p_penempatan   = isset($_POST['penempatan']) ? $_POST['penempatan'] : "";
    $maxsize        = 2097152;
    ...
    if ($p_tombol == "DAFTAR") {
        $required_fields = array($p_nama, $p_nip, $p_tmptlahir, $p_tgl_lahir, $p_alamat, $p_datemasuk, $p_bag, $p_kdpndidik, $p_kdstatusk, $p_kdstatusp);
        ...
        if (in_array("", $required_fields)) {
            ...
            if ($j_cek_ganda > 0) {
                ...
                if ($_FILES['file']['size'] >= $maxsize) {
                    ...
                    $alamatfile = ($_FILES['file']['size'] == 0) ? 'assets/avatars/avatar5.png' : move_uploaded_file($_FILES['file']['tmp_name'], 'examples/' . $nama_file);

                    $q_daftar = mysqli_query($mysqli, "INSERT INTO pegawai VALUES ('0','$p_nip','$p_nama','$p_tmptlahir','$p_tgl_lahir',
                    ...
                    '$p_nofax','$p_email_k','$p_nodplk','$p_nojiwa','$p_nidn','$p_stsk','$p_datekontrak','$p_id_prov','$p_id_kabkot','$p_id_kec','$p_penempatan',NOW())");
        ...
        if (
            $p_nama == "" || $p_nip == "" || $p_tmptlahir == "" || $p_tgl_lahir == "" || $p_alamat == ""
        ) {
            echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><i class='icon-remove'></i></button><strong><i class='ace-icon fa fa-times'></i> MAAF! </strong> Form Isian Masih Belum lengkap mohon di lengkapi, <a href='?id=tambahpeg'>Kembali</a><br/></div>";
        } else {
            ...
            if (($_FILES['file']['size'] >= $maxsize)) {
                ...
                if (($_FILES['file']['size'] == 0)) {
                    $q_edit = mysqli_query($mysqli, "UPDATE pegawai SET nip='$p_nip', nama='$p_nama',tmpt_lahir='$p_tmptlahir',tgl_lahir='$p_tgl_lahir',
									...
									tlpn='$p_tlpn',nohp='$p_hp',npwp='$p_nonpwp',id_agm='$p_id_agm',kdpndidik='$p_kdpndidik',noktp='$p_noktp',nojamsos='$p_nojamsos',
									...
									id_prov='$p_id_prov',id_kabkot='$p_id_kabkot',id_kec='$p_id_kec',klien='$p_penempatan',
                    ...
                    <form class="form-horizontal" action="?id=tambahpeg" method="post" id="tambahpegawai" role="form" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <div id="image_preview">
                                    <img id="previewing" src="<?php echo $p_foto; ?>" width="250" height="230" />
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-10 col-sm-9">
                                <textarea name="alamat" placeholder="Mohon Isi Alamat dengan lengkap..." required><?= $p_alamat ?></textarea>
                                <select name="propinsi" id="propinsi">
                                    <option>--Pilih Provinsi--</option>
                                    <?php foreach ($mysqli->query("SELECT * FROM prov ORDER BY nama_prov") as $p) : ?>
                                        <option value="<?= $p['id_prov'] ?>" <?= $p['id_prov'] == $p_id_prov ? 'selected' : '' ?>><?= $p['nama_prov'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <select name="kota" id="kota">
                                    ...
                                    <?php
                                    // Sanitasi input
                                    $p_id_prov = filter_var($p_id_prov, FILTER_SANITIZE_FULL_SPECIAL_CHARS);

                                    // Persiapkan statement
                                    if ($stmt = $mysqli->prepare("SELECT * FROM kabkot WHERE id_prov= ? ORDER BY nama_kabkot")) {
                                        // Bind parameter
                                        $stmt->bind_param("s", $p_id_prov);
                                        $stmt->execute();
                                        ...
                                        if ($result->num_rows > 0) {
                                            echo "<!-- Ditemukan " . $result->num_rows . " baris -->";
                                        } else {
                                            echo "<!-- Tidak ada hasil -->";
                                        }

                                        // Loop melalui hasil dan buat option
                                        while ($p = $result->fetch_assoc()) {
                                    ?>
                                            <option value="<?= htmlspecialchars($p['id_kabkot']) ?>" <?= $p['id_kabkot'] == $p_id_kabkot ? 'selected' : '' ?>><?= htmlspecialchars($p['nama_kabkot']) ?></option>
                                    <?php
                                        }
                                        ...
                                        echo "<!-- Terjadi kesalahan pada prepare statement -->";
                                    }
                                    ?>
                                </select>
                                ...
                                <select name="kecamatan" id="kecamatan">
                                    ...
                                    <?php foreach ($mysqli->query("SELECT * FROM kec WHERE id_prov=$p_id_prov AND id_kabkot=$p_id_kabkot ORDER BY nama_kec") as $p) : ?>
                                        <option value="<?= $p['id_kec'] ?>" <?= $p['id_kec'] == $p_id_kec ? 'selected' : '' ?>><?= $p['nama_kec'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right" for="tlpn">Tlp :</label>

                            <div class="col-xs-8 col-sm-9">
                                <input type="text" name="tlpn" id="tlpn" class="col-xs-10 col-sm-2" placeholder="Nomor WA" value="<?php echo $p_tlpn; ?>" />
                                <input class="col-xs-10 col-sm-2" value="<?php echo $p_hp; ?>" type="text" name="hp" id="hp" placeholder="Isikan No Hp">
                                <input class="col-xs-10 col-sm-2" value="<?php echo $p_nofax; ?>" type="text" name="fax" id="fax" placeholder="No Darurat">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                ...
                                <select name="penempatan" value="">
                                    ...
                                    while ($ak = mysqli_fetch_array($q)) {
                                        if ($ak[2] == $p_penempatan) {
                                            echo "<option value='$ak[2]' selected>$ak[2]</option>";
                                        } else {
                                            echo "<option value='$ak[2]'>$ak[2]</option>";
                                        }
                                    }
                                    ?>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <span>
                                    ...
                                    <input type="text" name="nojamsos" value="<?php echo $p_nojamsos; ?>" id="nojamsos" placeholder="No.BPJS TK" />
