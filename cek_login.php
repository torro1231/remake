		...
		if (password_verify($pass, $passhash)) {
			...
			if ($sisa < 0) {
				...
				if ($_SESSION['leveluser'] == 1) {
					...
					echo "<div id='sukses' class='alert alert-info'><strong>BERHASIL...</strong><button type='button' class='close' data-dismiss='alert'><i class='ace-icon fa fa-times'></i></button></div><script>window.location ='prinsipal.php?id=home'</script>";
					mysqli_query($mysqli, "update tbl_user set status=1,w_login=NOW() where id_user='$id_user'");
				}
			}
		} else {
